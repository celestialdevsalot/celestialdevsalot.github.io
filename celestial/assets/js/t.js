...
fetch("/celestial/assets/json/tools.json")
  .then(res => res.json())
  .then(games => {
    function showGames(list) {
      ...
      list.forEach(g => {
        ...
        card.innerHTML = `<div class="thumb" style="background-image:url('${g.img || "/celestial/assets/img/placeholder.png"}')"></div><p>${g.name}</p>`;
